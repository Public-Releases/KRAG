...
def remove_ground_truth_fields(patient_case):
    cleaned_case = patient_case.copy()
    fields_to_remove = [
        "Processed Diagnosis",
        "PATHOLOGY", 
    ...
    for field in fields_to_remove:
        cleaned_case.pop(field, None)
    return cleaned_case

def get_embeddings(texts, use_local=True):
    if use_local:
...
def get_query_embedding(query, use_local=True):
    """Get embedding for a single query"""
    return get_embeddings([query], use_local=use_local)[0]
...
def generate_diagnosis_report(path, query, retrieved_documents, i,top_n,match_n,model):
    ...
    if not hasattr(generate_diagnosis_report, 'ddxplus_retriever'):
        generate_diagnosis_report.ddxplus_retriever = DDXPlusKGRetriever(kg_path=path_str)
    
    additional_info, clean_symptoms, winning_category = get_additional_info_ddxplus(
        generate_diagnosis_report.ddxplus_retriever, 
        query,
        top_n=top_n
    )
    ...
    if model.startswith('local/'):
        ...
        model_map = {
            ...
            'local/llama-2-7b': 'meta-llama/Llama-2-7b-chat-hf',
            'local/llama-2-13b': 'meta-llama/Llama-2-13b-chat-hf',
            'local/mistral-7b': 'mistralai/Mistral-7B-Instruct-v0.2',
            'local/mixtral-8x7b': 'mistralai/Mixtral-8x7B-Instruct-v0.1',
            'local/openbiollm-8b': 'aaditya/Llama3-OpenBioLLM-8B',
        }
        
        hf_model_name = model_map.get(model, 'meta-llama/Meta-Llama-3.1-8B-Instruct')
        ...
        return response, additional_info, clean_symptoms, winning_category
    
    elif model in ['gpt-4o', 'gpt-4o-mini', 'gpt-3.5-turbo-0125']:
        ...
        return response.choices[0].message.content, additional_info, clean_symptoms, winning_category
...
document_embeddings_file_path='./dataset/document_embeddings_ddxplus.npy'
...
if os.path.exists(document_embeddings_file_path):
    ...
    print("Generating document embeddings using local model (FREE)...")
    document_embeddings = get_embeddings(documents, use_local=True)
    save_embeddings(document_embeddings, document_embeddings_file_path)
else:
    print("No training documents found - embeddings will be generated when needed")
